...
@author: Administer
"""
def result_prediction(seq_id, output_file):
    import numpy as np
    import pandas as pd
    import os
    import sys
    ...
    for model_num in range(1,11,1):
        ...
        x_test=pd.read_csv('./features/combined_features/feature_'+str(model_num)+'.csv',sep=',',header=0,index_col=0).to_numpy()
    ...
    sss=list(map(lambda x: 1 if x>0.5 else 0,sss)) 
    sss=np.array(sss).reshape(x_test_length,10)
    pred_class=np.sum(np.array(sss),axis=1)    
    pred_class=list(map(lambda x: "ac4C Site" if x>5 else "Non-ac4C Site",pred_class))
