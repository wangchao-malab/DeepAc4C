...
def w2v_kmer_corpus():
    with open('./sequence/w2v_kmer.txt',"w") as fil1:
            for seq_record in SeqIO.parse('./sequence/input_query.fasta', "fasta"):
                seq_id=seq_record.id
                seq=seq_record.seq
                sub_list=kmer_seq(seq,4)
...
def w2v_features():
    ...
    for i in range(1,11,1):
        model= Word2Vec.load("./model/w2v/cd_ac4c_training_"+str(i)+"_k41_shffule.model")       
        X=word_vector(np.array(list_seq),model)
        pd.DataFrame(X).to_csv("features/mm/"+str(i)+"/f_b/w2v_feature.csv",sep=",")   